  ...
  const handleSubmit = async (e) => {
    ...
    if (Object.keys(errors).length === 0) {
      try {
        ...
        localStorage.setItem("dresstoken", response?.access); // Store token in localStorage
        toast.success("Login successful!");

        // Use a timeout instead of setInterval for delayed navigation
        setTimeout(() => {
          navigate("/category"); // Redirect after login
    ...
    <section className="min-h-screen flex items-center justify-center  px-4 bg-blue-50">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="flex items-center justify-between text-sm mb-6">
          ...
          {/* <Link to="/forget_password" className="text-primary hover:underline">
            Forgot Password?
          </Link> */}
        </div>
