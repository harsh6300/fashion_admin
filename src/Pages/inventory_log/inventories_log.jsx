...
import { useGetProfileQuery, useGetWarehouseQuery, useGetInventoriesQuery, useGetProductQuery, useDeleteInventoriesMutation, useGetInventoylogQuery, useDeleteInventorylogMutation } from "../../services/apiSlice";
...
const Category = () => {
    ...
    const { data: categories } = useGetInventoylogQuery();
    const [deleteinvetory_log] = useDeleteInventorylogMutation();
    ...
    const handleDeleteCategory = async () => {
        try {
            ...
            formdata.append('inventory_log_id', deletcategory)

            await deleteinvetory_log(formdata).unwrap();
        ...
        <div>
            ...
            <div className='flex inter'>
                ...
                <div className="w-full width__right relative max-md:ms-0">
                    <SubHeader setIsOpenside={setIsOpenside} pageName={"inventories Log"} />
                    <div className="flex justify-between gap-[10px] mb-[27px] flex-wrap items-center max-sm:block" >
                        ...
                        <nav className="text-sm text-gray-600 max-sm:mt-[15px] max-sm:ps-[5px]" aria-label="Breadcrumb" >
                            <ol className="flex flex-wrap items-center ">
                                ...
                                <li className="text-primary font-medium text-[12px]">All inventories Log</li>
                            </ol>
                        </nav>
                    ...
                    <div className="border mt-[30px] bg-white border-[#EBE9F1] shadow-[0px_4px_24px_0px_#0000000F]  rounded-[10px]">
                        <div className="p-[20px] flex  gap-4 max-lg:items-center justify-between">
                            ...
                            <div>
                                <h3 className="font-[600] text-[16px] text-[#313b5e] hans">All inventories Log List</h3>
                            </div>
                            <div className="flex gap-2">
                                ...
                                <div>
                                    <Link to="/Create_inventory_log">
                                        <button className="bg-[#ff6c2f] text-white text-[12.6px] rounded-[8px] py-[5.56px] px-[12px]">Add Inventories Log</button>
                                    </Link>
                                </div>
                                <div>
                            ...
                            <table className="w-full ">
                                <thead className="border-y border-[#ddd]">
                                    ...
                                    <tr className="max-sm:h-[40px] h-[54px] ">
                                        ...
                                        <th className="text-[14px] font-bold text-center text-[#5d7186] sans text-nowrap max-lg:hidden max-md:text-[13px] max-sm:px-[10px]">Inventory</th>
                                        <th className="text-[14px] font-bold text-center text-[#5d7186] sans text-nowrap max-md:hidden max-md:text-[13px] max-sm:px-[10px]">Quantity Changed</th>
                                        ...
                                        <tr
                                            ...
                                            className="h-[44px] sm:h-[58px] transition-all duration-200 bg-white hover:bg-[rgba(238,243,249,0.8)] border-b border-[#ddd]" onClick={(e) => {
                                                ...
                                                navigate("/edit_inventory", {
                                                    state: { inventory_log_id: item?.inventory_log_id },
                                                });
                                            }} s
                                        >

                                            <td className="text-sm text-[#5E5873] text-center px-[30px]  max-xl:px-[10px] max-sm:px-[5px] max-md:hidden max-md:text-[12px]">{item.inventory}</td>
                                            <td className="text-sm text-[#5E5873] text-center px-[30px]  max-xl:px-[10px] max-sm:px-[5px] max-md:hidden max-md:text-[12px]">{item.quantity_changed}</td>
                                            <td className="text-sm text-[#5E5873] text-center px-[30px]  max-xl:px-[10px] max-sm:px-[5px] max-md:hidden max-md:text-[12px]">{item.reason}</td>
                                            ...
                                            <td className="text-sm text-[#5E5873] text-center px-[30px]  max-xl:px-[10px] max-sm:px-[5px] text-nowrap max-md:text-[12px] max-sm:hidden">
                                                <button
                                                    className={`px-[16px] py-1 rounded-[400px] 
        ${item?.change_type === "subtraction" ? "bg-[#ff6c2f1a] text-[#ff6c2f]" : ""}
        ${item?.change_type === "OUT" ? "bg-[#edf0f2] text-[#62778c]" : ""}
        ${item?.change_type === "IN" ? "bg-[#22c55e1a] text-[#22c55e]" : ""}
    `}
                                                >
                                                    {item?.change_type}
                                                </button>
                                            ...
                                            <td className="text-sm text-[#5E5873] text-center px-[30px] max-xl:px-[10px] max-sm:px-[5px]">
                                                <div className="flex gap-2 justify-center max-md:gap-1 max-sm:gap-0">
                                                    ...
                                                    <button className="w-[44px] h-[32px] max-md:h-[35px] max-md:w-[35px] bg-[#ff6c2f1a] flex justify-center items-center text-primary hover:bg-primary hover:text-white rounded-[0.5rem]" onClick={(e) => {
                                                        e.stopPropagation();
                                                        openModal('modal3')
                                                        setDeletcategory(item?.inventory_log_id);
                                                    }}>
