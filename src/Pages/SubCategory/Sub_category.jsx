...
import { useGetSubCategoriesQuery, useDeleteSubCategoryMutation, useGetProfileQuery, } from "../../services/apiSlice";
...
const Sub_Category = () => {
    ...
    // delete SubCategory
    const [deletSubcategory, setDeletSubcategory] = useState(null);
    const handleDeleteCategory = async () => {
        try {
            const formdata = new FormData()

            formdata.append('subcategory_id', deletSubcategory)
    ...
    const filteredData = subcategoriesData?.filter(item =>
    ...
    const handleImageChange = (e) => {
        ...
        if (file) {
            ...
            setsubCategory((prev) => ({ ...prev, file })); // Store the file object
        }
    };
        ...
        <div>
            ...
            <div className='flex inter'>
                ...
                <div className="w-full width__right relative max-md:ms-0">
                    <SubHeader setIsOpenside={setIsOpenside} pageName={"SubCategory"} />
                    <div className="flex justify-between gap-[10px] mb-[27px] flex-wrap items-center max-sm:block" >
                        <h3 className="text-[22px] font-[700] text-[#707793] uppercase "> SubCategory</h3>
                    ...
                    <div className="border mt-[30px] bg-white border-[#EBE9F1] shadow-[0px_4px_24px_0px_#0000000F]  rounded-[10px]">
                        <div className="p-[20px] flex  gap-4 items-center justify-between">
                            ...
                            <div>
                                <h3 className="font-[600] text-[16px] text-[#313b5e] hans">All SubCategories List</h3>
                            </div>
                            <div className="flex gap-2">
                                ...
                                <div>
                                    <Link to="/create_subcategory">
                                        <button className="bg-[#ff6c2f] text-white text-[12.6px] rounded-[8px] py-[5.56px] px-[12px]">Add SubCategory</button>
                                    </Link>
                                </div>
                                <div>
                                    ...
                                    {displayedData.map((item, index) => (
                                        <tr
                                            ...
                                            className="h-[44px] sm:h-[58px] transition-all duration-200 bg-white hover:bg-[rgba(238,243,249,0.8)] border-b border-[#ddd]" onClick={(e) => {
                                                e.stopPropagation();
                                                navigate("/edit_sucategory", {
                                                    state: { subcategory_id: item?.subcategory_id },
                                                });
                                            }}
                                        >
                                            <td className="text-sm text-[#5E5873] flex  px-[30px] max-xl:px-[10px] max-sm:px-[5px] max-sm:min-w-[120px]   items-center gap-2 my-[15px] max-md:gap-1 max-md:text-[12px]">
                                                ...
                                                <div className="h-[56px] min-w-[56px] max-md:min-w-[40px] max-md:max-h-[40px] bg-[#eef2f7] rounded-[12px] flex items-center justify-center overflow-hidden">
                                                    <img src={`${import.meta.env.VITE_API_BASE_URL}${item.subcategory_img}`} alt={`${item.name}`} className="object-cover h-[56px] w-[56px] max-md:w-[40px] max-md:h-[40px]" />
                                                </div>
                                            ...
                                            <td className="text-sm text-[#5E5873] text-center px-[30px]  max-xl:px-[10px] max-sm:px-[5px]  max-lg:hidden max-md:text-[12px]">{item.description}</td>
                                            <td className="text-sm text-[#5E5873] text-center px-[30px]  max-xl:px-[10px] max-sm:px-[5px] text-nowrap max-md:text-[12px] max-sm:hidden">
                                                <button
                                                    className={`px-[16px] py-1 rounded-[400px] ${item?.is_home
                                            ...
                                            <td className="text-sm text-[#5E5873] text-center px-[30px] max-xl:px-[10px] max-sm:px-[5px]">
                                                <div className="flex gap-2 justify-center max-md:gap-1 max-sm:gap-0">
                                                    ...
                                                    <button className="w-[44px] h-[32px] max-md:h-[35px] max-md:w-[35px] bg-[#ff6c2f1a] flex justify-center items-center text-primary hover:bg-primary hover:text-white rounded-[0.5rem]" onClick={(e) => {
                                                        e.stopPropagation();
                                                        openModal('modal3')
                                                        setDeletSubcategory(item?.subcategory_id);
                                                    }}>
                        ...
                        {subcategoriesData.length > itemsPerPage && (
