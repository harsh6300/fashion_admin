...
import { useGetProfileQuery, useGetRoleQuery, useDeleteRoleMutation, useGetDepartmentQuery, useDeleteDepartmentMutation, } from "../../services/apiSlice";
...
const Fabric = () => {
    ...
    const { data: categories } = useGetDepartmentQuery();
    ...
    const handleDeleteCategory = async () => {
        try {
            ...
            formdata.append('department_id', deletcategory)

            await deleteDepartment(formdata).unwrap();
        ...
        <div>
            ...
            <div className='flex inter'>
                ...
                <div className="w-full width__right relative max-md:ms-0">
                    <SubHeader setIsOpenside={setIsOpenside} pageName={"Department"} />
                    <div className="flex justify-between gap-[10px] mb-[27px] flex-wrap items-center max-sm:block" >
                        <h3 className="text-[22px] font-[700] text-[#707793] uppercase "> Department</h3>

                        <nav className="text-sm text-gray-600 max-sm:mt-[15px] max-sm:ps-[5px]" aria-label="Breadcrumb" >
                            <ol className="flex flex-wrap items-center ">

                                
                                <li className="text-primary font-medium text-[12px]">All Department</li>
                            </ol>
                        </nav>
                    ...
                    <div className="border mt-[30px] bg-white border-[#EBE9F1] shadow-[0px_4px_24px_0px_#0000000F]  rounded-[10px]">
                        <div className="p-[20px] flex  gap-4 max-lg:items-center justify-between">
                            ...
                            <div>
                                <h3 className="font-[600] text-[16px] text-[#313b5e] hans">All Department List</h3>
                            </div>
                            <div className="flex gap-2">
                                ...
                                <div>
                                    <Link to="/create_department">
                                        <button className="bg-[#ff6c2f] text-white text-[12.6px] rounded-[8px] py-[5.56px] px-[12px]">Add Department</button>
                                    </Link>
                                </div>
                                <div>
                            ...
                            <table className="w-full ">
                                <thead className="border-y border-[#ddd]">
                                    ...
                                    <tr className="max-sm:h-[40px] h-[54px] ">
                                        ...
                                        <th className="text-[14px] font-bold text-center text-[#5d7186] sans text-nowrap max-md:text-[13px] max-sm:px-[10px]">Description</th>
                                ...
                                <tbody className="">
                                    {categories1.map((item, index) => (
                                        <tr
                                            key={index}
                                            className="h-[44px] sm:h-[58px] transition-all duration-200 bg-white hover:bg-[rgba(238,243,249,0.8)] border-b border-[#ddd]" onClick={() => navigate("/edit_department", { state: { department_id: item.department_id } })}
                                        >
                                            <td className="text-sm text-[#5E5873] flex  px-[30px] max-xl:px-[10px] max-sm:px-[5px] max-sm:min-w-[120px]   items-center gap-2 my-[15px] max-md:gap-1 max-md:text-[12px]">
                                                {/* <div className="flex justify-center  rounded-[12px] max-sm:hidden">
                                                    <label className="cursor-pointer flex items-center ">
                                                        <div
                                                            className={`checkbox w-[20px] h-[20px]   rounded-[4px] bg-[#eef2f7] flex items-center justify-center transition-colors ${checkedItems[item.id] ? 'bg-[#ff6c2f] border-ring-[#ff6c2f]' : ' border-2 border-[#ece7e7f1]'
                                                                }`}
                                                            onClick={(e) => {
                                                        ...
                                                        >
                                                            <span
                                                                className={`checkmark bg-[#ff6c2f] ${checkedItems[item.id] ? 'opacity-100' : 'opacity-0'
                                                                    } flex items-center justify-center text-white rounded-[3px] transition-opacity`}
                                                            >
                                                                ✔
                                                            </span>
                                            ...
                                            <td className="text-sm text-[#5E5873] text-center px-[30px]  max-xl:px-[10px] max-sm:px-[5px] max-sm:hidden max-md:text-[12px]">{item.description}</td>
                                            <td className="text-sm text-[#5E5873] text-center px-[30px]  max-xl:px-[10px] max-sm:px-[5px] max-md:text-[12px]">
                                                <button
                                                    className={`px-[16px] py-1 rounded-[400px] ${item?.is_active 
                                            ...
                                            <td className="text-sm text-[#5E5873] text-center px-[30px] max-xl:px-[10px] max-sm:px-[5px]">
                                                <div className="flex gap-2 justify-center max-md:gap-1 max-sm:gap-0">
                                                    ...
                                                    <button onClick={(e) => {
                                                        openModal('modal3')
                                                        e.stopPropagation(); // Prevent row click event
                                                        setDeletcategory(item.department_id)
                            ...
                            <div className="flex min-h-full items-center justify-center p-4 text-center sm:items-center sm:p-0">
                                <div className="relative transform overflow-hidden p-[30px] border-t-[8px] border-[#F44336] rounded-[6px] bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-xl z-40">
                                    <div className="bg-white ">
                                        ...
                                        <p className="pt-[14px] text-[#8492A6]">Are you sure you want to delete this Fabric?</p>
