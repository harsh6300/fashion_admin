...
import { useEditDepartmentMutation, useGetDepartmentQuery } from '../../services/apiSlice';
...
const Create_role = () => {
    ...
    const { data: data1 } = useGetDepartmentQuery()

    const department = data1?.data?.find((val) => val.department_id == location.state?.department_id)
    ...
    useEffect(() => {
        if (department) {
            ...
            setStatus(department?.is_active ? 'active' : 'inactive');
        }
    }, [department]);
    ...
    const handleSubmit = async () => {
        try {
            ...
            formData.append("department_id", location.state.department_id);
            formData.append("name", value);
            formData.append("description", name);
            formData.append("is_active", status == 'active' ? 'True' : 'False');


            const response = await editdepartment({ formData }).unwrap();
        ...
        <div>
            ...
            <div className='flex inter'>
                ...
                <div className="w-full width__right relative max-md:ms-0">
                    <SubHeader setIsOpenside={setIsOpenside} pageName={"Department Edit"} />
                    <div className="flex justify-between gap-[10px] mb-[50px] flex-wrap max-sm:mb-[20px]" >
                        <h4 className="text-[18px] text-gray font-semibold">Department Edit</h4>
                    </div>
