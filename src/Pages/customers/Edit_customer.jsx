...
import { useAddCategoryMutation, useAddCustomerMutation, useEditCustomerMutation } from "../../services/apiSlice";
...
const Create_customer = () => {
    ...
    useEffect(() => {
        if (location?.state?.customer_id) {
            setForm({
                name:location?.state?.customer_id?.name,
                address: location?.state?.customer_id?.address,
                email: location?.state?.customer_id?.email,
                phone: location?.state?.customer_id?.phone,
                customer_id: location?.state?.customer_id?.customer_id
            })
        }
    ...
    const handlesubmit = async () => {
        ...
        try {
            ...
            const response = await editCustomer(formData).unwrap();
