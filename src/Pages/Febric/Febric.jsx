...
import { useGetCategoriesQuery, useDeleteCategoryMutation, useGetProfileQuery, useGetColorQuery, useDeleteColorMutation, useGetFebricQuery, useDeleteFebricMutation, } from "../../services/apiSlice";
...
const Fabric = () => {
    ...
    const { data: categories } = useGetFebricQuery();
    const [deleteColor] = useDeleteFebricMutation();
    ...
    const handleDeleteCategory = async () => {
        try {
            ...
            formdata.append('fabric_id', deletcategory)

            await deleteColor(formdata).unwrap();
            toast.success("Fabric deleted successfully!");
            closeModal();
        } catch (error) {
            toast.error(error.data?.message || error.data?.message || "Failed to delete Fabric");
        }
    };
                ...
                <div className="w-full width__right relative max-md:ms-0">
                    ...
                    <div className="border mt-[30px] bg-white border-[#EBE9F1] shadow-[0px_4px_24px_0px_#0000000F]  rounded-[10px]">
                        <div className="p-[20px] flex  gap-4 items-center justify-between">
                            ...
                            <div>
                                <h3 className="font-[600] text-[16px] text-[#313b5e] hans">All Fabric List</h3>
                            </div>
                            <div className="flex gap-2">
                                ...
                                <div>
                                    <Link to="/create_fabric">
                                        <button className="bg-[#ff6c2f] text-white text-[12.6px] rounded-[8px] py-[5.56px] px-[12px]">Add Fabric</button>
                                    </Link>
                                </div>
                                <div>
                            ...
                            <table className="w-full ">
                                ...
                                <tbody className="">
                                    {displayedData.map((item, index) => (
                                        <tr
                                            key={index}
                                            className="h-[44px] sm:h-[58px] transition-all duration-200 bg-white hover:bg-[rgba(238,243,249,0.8)] border-b border-[#ddd]" onClick={() => navigate("/edit_Fabric", { state: { fabric_id: item.fabric_id } })}
                                        >
                                            ...
                                            <td className="text-sm text-[#5E5873] text-center px-[30px] max-xl:px-[10px] max-sm:px-[5px]">
                                                <div className="flex gap-2 justify-center max-md:gap-1 max-sm:gap-0">
                                                    ...
                                                    <button onClick={(e) => {
                                                        ...
                                                        setDeletcategory(item.fabric_id)
