...
import {  useDeleteCategoryMutation, useGetProfileQuery, useGetColorQuery, useDeleteColorMutation, useGetFebricQuery, useDeleteFebricMutation, useGetOrderQuery, } from "../../services/apiSlice";
...
const Order = () => {
    ...
    const handleDeleteCategory = async () => {
        try {
            ...
            formdata.append('order_id', deletcategory)

            await deleteColor(formdata).unwrap();
            toast.success("Order deleted successfully!");
            closeModal();
        } catch (error) {
            toast.error(error.data?.message || error.data?.message || "Failed to delete Order");
        }
    };
    ...
    const toggleCheckbox = (id) => {
        setCheckedItems((prev) => {
            ...
            const allChecked = Order1.every(item => updated[item.category_id]);
            ...
            <div className='flex inter'>
                ...
                <div className="w-full width__right relative max-md:ms-0">
                    ...
                    <div className="flex justify-between gap-[10px] mb-[27px] flex-wrap items-center max-sm:block" >
                        <h3 className="text-[22px] font-[700] text-[#707793] uppercase "> Order</h3>

                        <nav className="text-sm text-gray-600 max-sm:mt-[15px] max-sm:ps-[5px]" aria-label="Breadcrumb" >
                            <ol className="flex flex-wrap items-center ">

                                
                                <li className="text-primary font-medium text-[12px]">All Order</li>
                            </ol>
                        </nav>
                            ...
                            <table className="w-full ">
                                <thead className="border-y border-[#ddd]">
                                    ...
                                    <tr className="max-sm:h-[40px] h-[54px] ">
                                        ...
                                        <th className="text-[14px] font-bold text-center text-[#5d7186] sans text-nowrap max-md:text-[13px] max-sm:px-[10px]">Total Amount</th>
                                ...
                                <tbody className="">
                                    {displayedData.map((item, index) => (
                                        ...
                                        >
                                            ...
                                            <td className="text-sm text-[#5E5873] text-center px-[30px]  max-xl:px-[10px] max-sm:px-[5px] max-md:text-[12px]">
                                                <button
                                                    className={`px-[16px] py-1 rounded-[400px] ${item?.payment_status == 'Completed'
                                            ...
                                            <td className="text-sm text-[#5E5873] text-center px-[30px]  max-xl:px-[10px] max-sm:px-[5px] max-md:text-[12px]">
                                                <button
                                                    className={`px-[16px] py-1 rounded-[400px] ${item?.status == 'Completed'
                                                        ? "bg-[#22c55e1a] text-[#22c55e]"
                                                        : "text-primary bg-[#ff6c2f1a] "
                                                        }`}
                                                >
